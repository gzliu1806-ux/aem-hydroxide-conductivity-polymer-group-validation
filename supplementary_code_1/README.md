...
Supplementary Code 1 shows the public field layout, polymer-group split, row-wise control, and feature-family loop on a synthetic 10-row dataset. It is meant to make the validation logic inspectable while keeping the literature extraction table on the manuscript data-access route.

## Contents

- `README.md`: this run guide.
- `PUBLIC_READER_GUIDE.md`: a short guide to the runnable example, manuscript metric index, claim file, question file, and figure file.
...
- `figure_source_map_public.json`: a figure-level source map for Fig. 1-Fig. 7 and Fig. S1-Fig. S8; it maps each figure to artwork files, `journal_upload_artifacts`, manuscript/SI captions, Supplementary Data tables, entries, and boundary notes.
...
- Polymer-group full model: R² = 0.805, RMSE = 0.128, MAE = 0.096 (Supplementary Data 1, Table S6).
...
- Source-held-out offset check: mean-shift R² = 0.871 and affine R² = 0.891 (Supplementary Data 1, Table S6 and Fig. S8).
- Uncertainty and split-size summary: primary-model R² = 0.805 with 95% group-bootstrap CI 0.772 to 0.834; field-availability strata and split-size ranges are summarized in Supplementary Data 1, Table S13.
...
- The `figure_source_map_public.json` map covers all seven main figures and eight supplementary figures. It maps each figure to artwork files and public files while noting that exact row-level plotting data use the observation-level table.
